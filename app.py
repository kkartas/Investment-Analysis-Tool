            ...
            if data is not None:
                ...
                if data.empty:
                    ...
                    stock_chart_data = {
                        ...
                        'sma50': [safe_chart_val(val) for val in data['SMA_50'].tolist()],
                        'sma200': [safe_chart_val(val) for val in data['SMA_200'].tolist()],
                        'macd': [safe_chart_val(val) for val in data['MACD'].tolist()],
                        'rsi': [safe_chart_val(val) for val in data['RSI'].tolist()]
                    }
                    ...
                    total_inv, future_value, total_profit, data_points = dca_calculation(
                        data, init_inv, period_inv, (request.form.get('frequency') or 'monthly').lower(), dur, annual_return=calculated_exp_return
                    )
                    # Format DCA results with no decimals and thousand separator.
                    dca_result = {
